...
::: callout-note
L'obectif de ce tutoriel est de montrer comment utiliser JAX pour calculer le JVP (Jacobian Vector Product) et le VJP (Vector Jacobian Product) d'une fonction qui n'est pas disponible dans les primitives fournies par JAX/Torch. Les deux cas d'usage envisagés sont:
...
# Implémentation de l'exemple simple \[75 minutes\]
...
1. Sans passer par les fonctions forward et backward de JAX, implémenter la dérivée 
...
`jnp.arange(dim_sortie)` fournit les indices $i = 0, 1, ..., \text{dim_sortie}−1$ pour lesquels on veut calculer le gradient.
...
4.  Calculer la dérivée de notre fonction en utilisant les fonctions `jax.jvp` et `jax.vjp` fournies en JAX. 
5.  Redéfinissez *manuellement* les opérateurs `jvp` et `vjp` associés à notre exemple, comme si l'on chercher à créer un brique élémentaire pour la chaine de dérivation.

Vérifier à chaque fois que les différentes approches donnent le même résultat que la version analytique.

## Indice

4. Pour cela nous pouvons utiliser `jax.jvp(f, (x,), (v,))` et `jax.vjp(f, x)` (puis appliquer la fonction retournée par `jax.vjp` sur un vecteur cotangent `u`).
...
### 4. Jacobienne avec jax.jvp et jax.vjp
...
### 5. Redéfinition manuelle de jvp et vjp
...
jac_fwd_jit = jax.jit(jax.jacfwd(lambda x: f_jit(x), argnums=0))
jac_bwd_jit = jax.jit(jax.jacrev(lambda x: f_jit(x), argnums=0))
```
...
# Partie Hugo : Application au calcul de la hessienne \[30 minutes\]
...
H_f_x_4 = jax.vmap(lambda pair: jax.jvp(lambda x: jax.jvp(f_fixed, (x,), (jnp.eye(4)[pair[1]],))[1], (x,), (jnp.eye(4)[pair[0]],))[1], (0,))(cartesian_prod).T.reshape(2, 4, 4)
