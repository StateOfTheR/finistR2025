...
3. `jax.grad` calcule la dérivée d'une fonction par formule backward, mais elle ne s'applique qu’aux fonctions à valeurs dans $\mathbb{R}$.
Il faut utiliser `jax.vmap` pour appliquer la même fonction à toutes les composantes du vecteur de sortie.
...
(où il faut définir rigoureuseument le produit $H_f(\mathbf{x})\mathbf{v}$ comme dans [cet article](https://arxiv.org/pdf/1911.13292)). 
...
La
[documentation de JAX](https://docs.jax.dev/en/latest/notebooks/autodiff_cookbook.html#hessian-vector-products-with-grad-of-grad)
donne de exemples concrets d'utilisation du HVP.
...
Attention, les produits de l'équation précédente sont définis au sens de la proposition 1 de l'article cité précédemment.
De plus on remarque que dans le cas de $f_1$ application linéaire (c'est notre cas ici), le deuxième terme est nul.
