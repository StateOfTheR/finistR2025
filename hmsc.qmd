...
$$\mu_{j k}=\sum_l t_{j l} \gamma_{l k}$$

$t_{j l}$ is the value of trait $l$ for species $j$ and $\gamma_{l k}$ the effect of trait $l$ on response to covariate $k$.
...
& \tau_{\mathrm{j}}=\prod_{l=1}^j \delta_{\mathrm{l}}
\end{aligned}
$$
...
The $\phi_{\mathrm{mj}}$ terms control the local shrinkage of species loadings (specific to a dimension $m$) and the $\delta_l$ control the global shrinkage of species loadings (concerns all the loadings of a species). This formulation allows for uncertainty in the number of factors and sparsity structure. The amount of shrinkage increases rapidly with factor index $m$, and thus, the model assigns non-negligible factor loadings only for those factors that are strongly supported by the data. 

Note that the species assocations could be made covariate-dependent, but this is not described in the tutorial.
