...
print("A")
print(grad_df_dx(x))
print(jax.vjp(f, x, a, b))
print("A")
assert jnp.allclose(grad_df_dx(x), jax.vjp(f, x, a, b)[0])
assert all(jnp.allclose(grad_df_dab((a,b))[i], jax.vjp(x, a, b)[i]) for i in range(2))

print("All right!")
```

Note that redefining the VJP redefines the grad function as well as every call to f! (which passes by f_fwd)
