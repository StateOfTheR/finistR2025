...
- en mode forward (ou `jvp`): on commence par calculer $v = \nabla g(z)$ (aussi appelé `tangents`) et $g(z)$ et le gradient $J = \nabla f(g(z))$ avant de calculer le produit scalaire $J^\top v$. En pratique on écrit une fonction `jvp` $(x, v) \mapsto \nabla f(x)^\top v$ qui calcule directement le produit scalaire pour éviter d'avoir à matérialiser $\nabla f$. 
- en mode reverse (ou `vjp`): on rétro-propage le gradient en calculant $v = \frac{\partial h(z)}{\partial g(z)} = \nabla f(g(z))$ (:warning: il faut avoir calculé et stocké $g(z)$ au préalable) et $J = \frac{\partial g(z)}{\partial z}$ et on calcule le produit scalaire $v J$. En pratique, on écrit une fonction `vjp` (ou `backward`) $(x, v) \mapsto v \nabla g(x)$ qui calcule directement le produit scalaire pour éviter d'avoir à matérialiser $\nabla g$. 
...
class f_torch_manual(torch.autograd.Function):
    ...
    def setup_context(ctx, inputs, output):
        x, a, b = inputs
        scaling = 1 - output.pow(2) # \tanh' = 1 - \tanh^2
...
Un VJP est capable de dévoiler une ligne de la jacobienne à la fois. Cela va donc nous permettre de calculer toute la matrice jacobienne de $f$ en un seul appel à JVP car c'est une matrice à une seule ligne. Nous l'avons vu dans la section précédente où en fait, `jax.jacrec` doit en fait appeler tous ces JVPs (ce qui est fait de manière cachée à l'utilisateur).
